  ...
  function runTests (feathersClient) {
    ...
    describe(`${servicePath} service -- ${transport}`, function () {
      ...
      it('sets issuanceTxId on transaction when created', function (done) {
        const txId = 'f9204f1a33a60436f19d36b49c81eae9b4f28aa82a51ed77c61c620c26aff02a'
        userUtils.authenticateTemp(app, feathersClient, this.user)
          ...
          .then(() => {
            return app.service('companies').create({
              name: 'Test company',
              registrationNumber: '9001',
              domicile: 'North Pole',
      ...
      it('associates with a company when created', function (done) {
        const txId = 'f9204f1a33a60436f19d36b49c81eae9b4f28aa82a51ed77c61c620c26aff02f'
        userUtils.authenticateTemp(app, feathersClient, this.user)
          .then(() => {
            return app.service('transactions').create({
              fromAddress: '000000000000000000000000',
          ...
          .then(issuance => {
            const issuanceId = issuance._id
            return app.service('issuances').find({ query: { _id: issuanceId } })
              .then(result => {
                const issuance = result.data[0]
                assert.equal(issuance.companyName, 'Test company', 'companyName assigned')
                assert.equal(issuance.domicile, 'North Pole', 'domicile assigned')
              }).then(() => done())
          })
          .catch(done)
      })
