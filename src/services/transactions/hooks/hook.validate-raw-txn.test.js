const app = require('../../../app')
const assert = require('assert')
const txnUtils = require('../../../../test-utils/transactions')
const testHook = require('./hook.validate-txn')

const validateRawTxn = testHook({
  btc: { url: 'http://localhost:18332', username: 'test', password: 'test' },
  eqb: { url: 'http://localhost:18332', username: 'test', password: 'test' }
})

describe('Transactions Service - validateRawTxn Hook', function () {
  before(function () {
    txnUtils.setupMock()
  })
  after(function () {
    txnUtils.resetMock()
  })

  it('uses the core to validate a raw transaction', async function () {
    const context = {
      app,
      params: {
        decodedTxn: txnUtils.decodedTxn
      },
      data: {
        fromAddress: 'mwmTx2oTzkbQg9spp6F5ExFVeibXwwHF32',
        addressTxid: '2ac0daff49a4ff82a35a4864797f99f23c396b0529c5ba1e04b3d7b97521feba',
        addressVout: 0,
        currencyType: 'BTC',
        toAddress: 'n2sYZjQFTC7vYLWSqX9Q8fjgtr5kb6GxYH',
        hex: ``
      }
    }

    await validateRawTxn(context)
      assert(context.params.passedValidation, 'the transaction was valid')
      // .catch(error => {
      //   console.log('ERROR ', error)
      //   assert(!error, error.message)
      //   done()
      // })
      // .catch(done)
  })

  it('requires otherAddress', function (done) {
    const context = {
      app,
      params: { },
      data: {
        address: 'mwmTx2oTzkbQg9spp6F5ExFVeibXwwHF32',
        addressTxid: '2ac0daff49a4ff82a35a4864797f99f23c396b0529c5ba1e04b3d7b97521feba',
        addressVout: 0,
        currencyType: 'BTC',
        hex: ``
      }
    }

    validateRawTxn(context)
      .then(context => {
        assert(!context, 'should have received bad request error')
        done()
      })
      .catch(error => {
        assert(error.className === 'bad-request', 'returned correct error class')
        assert(error.code === 400, 'returned correct error code')
        done()
      })
  })
})
